...
Population::Population(int pop_number, float mutation_probability, float(*function)(vector<float>, int), int function_dimension, float a, float b, int precision, float C)
{
	...
	this->crossover_probability = 0.3;
...
void Population::calc_fitness_schwefel()
{
	fitness.clear();
	for (int i = 0; i < pop_number; i++) {
		DNA candidate = population[i];

		float candidate_fitness = get_function_value(candidate.get_gene());
		candidate_fitness *= -1;
		candidate_fitness /= 1000;
		candidate_fitness = exp(candidate_fitness);

		fitness.push_back(candidate_fitness);
	}
}

void Population::calc_fitness_michalewicz()
{
	fitness.clear();
	for (int i = 0; i < pop_number; i++) {
		DNA candidate = population[i];

		float candidate_fitness = pow(get_function_value(candidate.get_gene()), 16);
		fitness.push_back(candidate_fitness);
	}
}
...
void Population::generate()
{
	...
	for (int i = 1; i < this->pop_number; i++) {
		...
		float p1 = rand.rand_uniform();
		float p2 = rand.rand_uniform();
		DNA child = cut_crossover(parent1, parent2, rand.rand_int(gene_dimension));
		child.mutate(this->rand.rand_int(this->gene_dimension));
		//child = crossover(parent1, parent2, fitness[x], fitness[y]);
		//child = cut_crossover(child, population[rand.rand_int(pop_number)], rand.rand_int(gene_dimension));
		children.push_back(child);
		/*if (p1 <= this->crossover_probability && p2 <= this->crossover_probability) {
			...
			if (get_function_value(parent1.get_gene()) > get_function_value(parent2.get_gene())) {
				DNA child = parent1;
				child.mutate(this->rand.rand_int(this->gene_dimension));
				children.push_back(child);
			}
	...
	population = children;
	//this->mutation_probability *= 0.9965;
	//this->crossover_probability *= 0.995;
}
